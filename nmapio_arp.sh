...
echo "[+] executing arp scanning "
# run nmap arp scan IMPORTANT: current AGGRESSIVE Tweak -T and parallelism for performance-safety balance
# sudo nmap -PR -v -T5 --min-parallelism 100 -oX $filename $ip
# nmap -PR --unprivileged -oX $filename $ip
sudo nmap -PR -oX $filename $ip
